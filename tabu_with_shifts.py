...
def compute_cost(solution, days, shifts, slots_per_day, demand, weightOverCover, weightShiftInstances):
    ...
    for oc in over_coverage:
        if oc < 0:
            under_cov -= oc
    under_cov_penalty = under_cov * time_interval_in_Minutes * weightOverCover * 10000
    ...
    average_shift_length_penalty = max((int(minimumAverageShiftLength * time_interval_in_Minutes) * total_shifts - total_length * time_interval_in_Minutes) * 10000, 0)
    ...
    return (cost, total_over_coverage, total_shift_instances)
...
if __name__ == "__main__":
    ...
    if path_data is None:
        #path_data = "instances/Toys/toy2.dzn"
        #nr = 6
    ...
    (cost, total_over_coverage, total_shift_instances) = compute_cost(solution, days, shifts, slots_per_day, demand, weightOverCover, weightShiftInstances)
    print("cost = ", cost)
    ...
    while loop < 1000000:
        ...
        (new_cost, new_total_over_coverage, new_total_shift_instances) = compute_cost(new_solution, days, shifts, slots_per_day, demand, weightOverCover, weightShiftInstances)
